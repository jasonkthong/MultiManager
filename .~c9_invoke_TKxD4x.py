...
def fulfillment():
    if request.method == "GET":
        ...
        if session["order"] != None:
            ...
            db.execute("UPDATE orders SET fulfilled=:value, inlockerat=:time, lockernum=:lockernum WHERE order_id=:order_id", order_id=order_id, value='1', time=, lockernum=request.form.get("name"))
