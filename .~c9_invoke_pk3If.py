...
def history():
    if request.method == 'GET':
        if session['role'] != 'Exec':
            ...
            orders = db.execute("SELECT orders.user_id, orders.pitch_desc, orders.camera, orders.mic, orders.lens, orders.tripod, orders.pickedupat, orders.returnedat, orders.completedat, orders.lockernum, photogs.first, photogs.middle, photogs.las FROM orders")
