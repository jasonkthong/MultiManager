...
def dayslot():
        if request.method == 'GET':
            if session['role'] != 'Exec':
                ...
                openorders = db.execute("SELECT orders.order_id, photogs.first, photogs.middle, photogs.last, orders.pitch_desc FROM orders INNER JOIN photogs ON photogs.id = orders.user_id WHERE ")
